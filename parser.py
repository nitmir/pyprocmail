...
assignement = variable + ~NL + '=' + ~NL + (QuotedString('"', "\\") | QuotedString("'", "\\") | Word(unicodePrintables))
assignements = (OneOrMore(Group(assignement)) + Optional(end_of_line)).setResultsName('assignements')

substitution = Literal('$') + ~NL + variable
substitution |= Literal('$\\') + ~NL + variable
substitution |= Literal('$') + ~NL + Literal('{') + ~NL + variable + Optional(~NL + Literal(':')) + ~NL + (Literal('-') | Literal('+')) + ~NL + Word(unicodePrintables.replace('}','')) + ~NL + Literal('}')
substitution |= QuotedString('`', "\\")
substitution = substitution.setResultsName('substitution')

recipe = Forward()

statement = ZeroOrMore(LineEnd()).suppress() + (comment | assignements | substitution | recipe) + ZeroOrMore(LineEnd()).suppress()
...
colon_line = (start_line + ~NL + Literal(':').suppress() + ~NL + Word(nums).setResultsName('number') + Optional(~NL + flags) + Optional(~NL + lockfile) + end_of_line).setResultsName('header')


condition = Forward()
condition_regex = Word(unicodePrintablesSpaces)
condition_size = ( Literal('>') | Literal('<')).setResultsName("sign") + Word(nums).setResultsName("size")
condition_shell = Literal('?').suppress() + Word(unicodePrintablesSpaces)
condition << (
               (variable.setResultsName("variable") + Literal('??') + condition.setResultsName("condition")).setResultsName("variable") | \
               ...
               (Word(nums).setResultsName("x") + Literal('^').suppress() + Word(nums).setResultsName("y") + condition.setResultsName("condition")).setResultsName("score") | \
...
action_shell = Optional(variable.setResultsName("variable") + ~NL + Literal('=')) + ~NL + Literal('|') + ~NL + Word(unicodePrintablesSpaces).setResultsName("cmd") + Optional(~NL + Literal('>>') + ~NL + Word(unicodePrintablesSpaces)).setResultsName('lockfile')
action_save = Word(unicodePrintablesSpaces.replace('{','').replace('!','').replace('|','').replace('*', ''), unicodePrintablesSpaces)
action_list = Literal('{').suppress() + statements + Literal('}').suppress()
action = (start_line + ~NL + (action_forward.setResultsName("forward") | action_list.setResultsName('statements') | action_shell.setResultsName("shell") | action_save.setResultsName("path")) + end_of_line).setResultsName('action')
